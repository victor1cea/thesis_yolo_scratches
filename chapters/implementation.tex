...
All the experiments will use the same test split and the relevant statistics about the splits are available in table \ref{impl:test_split_stats}.
...
\begin{tabular}{ ||c|c|c|c|c||}
\hline
window size & FP count & FN count & precision & recall\\ [0.5ex]
\hline\hline
320x320 & 0.8 & 0.68 & 0.91 & 0.1 \\
\hline
\end{tabular}
\caption{Overview of the stability of different windowing strategies/methods. The individual results are obtained by experiments with square windows of size 320, 640, 960 and 1280.}
\label{impl:win_size}
...
nalyzed, but this metric does not show the problems with the double detections. The double detections are generated, because smaller windows generate shorter scratches and sometimes the detections from two different windows do not overlap. In case of a double detection the IoU threshold is not reached and this leads to false positives. \\
